...
!
      parameter (nmnth=12, nx5min=4320, ny5min=216, nbatch = 10,
     #nadmin=10000, nluclass = 10000, nout = 5, rwetdcoef=0.5)
     ...
     #iadmin(ny5min,nx5min),mask(ny5min,nx5min)
      integer*4 itimeArray(3)    ! Holds the hour, minute, and second
      real rprc(nmnth,ny5min,nx5min),reto(nmnth,ny5min,nx5min),
     #rwet(nmnth,ny5min,nx5min),rcov(nmnth,ny5min,nx5min),
...
30    if (ibatch.eq.1) then
      ...
      do iy=1,ny5min
        read(ifilprc,*,end=40)(rprc(imnth,iy,ix),ix=1,nx5min)
...
!             perform calculation only if data are available
              if((rprc(i,j,k).gt.-99).and.(reto(i,j,k).gt.-99).and.
     #           (rwet(i,j,k).gt.-99).and.(rcov(i,j,k).gt.-99).and.
